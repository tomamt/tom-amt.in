...
@Component({
  selector: 'ngx-delivery-agent-da-unavailable',
  templateUrl: './delivery-agent-da-unavailable.component.html',
  styleUrls: ['./delivery-agent-da-unavailable.component.scss']
})
export class DeliveryAgentUnavailableComponent implements OnInit {
  ...
  socketLiveOrder(){
    ...
    this.socketSubscription = this.socketservices.getDABreakIssue().subscribe(message => {   
 ...
 orderstatus(data){
   if( this.page.currentPage == 1){
     ...
     this.removeByAttr(this.breakIssueList, '_id', oredrfiredData._id); 
     this.breakIssueList.unshift(oredrfiredData);
   }else{
     this.CnsumerIssuesList()
   }
 }
 removeByAttr (arr, attr, value){
  ...
  CnsumerIssuesList(){ 
    this.spinner.show();
    this.apiService.Issueslist('da-unavailable', this.page).subscribe((res)=>{
      this.spinner.hide();
      this.breakIssueList=res.issues;
        this.page = {
