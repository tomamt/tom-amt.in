...
@Component({
  selector: 'ngx-recipient-cancellations',
  templateUrl: './recipient-cancellations.component.html',
  styleUrls: ['./recipient-cancellations.component.scss']
})
export class RecipientCancellationsComponent implements OnInit {
  ...
  socketLiveOrder(){
    ...
    this.socketSubscription = this.socketservices.getrecipientCancellations().subscribe(message => {  
  ...
  CnsumerIssuesList(){
    this.spinner.show();
    this.apiService.Issueslist('consumer-issue', this.page).subscribe((res)=>{
      this.spinner.hide();
