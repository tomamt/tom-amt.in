...
@Component({
	selector: 'ngx-averagebar-chart-vea',
	templateUrl: './averagebar-chart-vea.component.html',
	styleUrls: ['./averagebar-chart-vea.component.scss']
})
export class AveragebarChartVEAComponent implements OnInit {
	private  bars:  any;
	data: DataModel[] = [];
	private tooltip : any;
	...
	private createChart(): void {
		...
		//x2.domain(data.map(function(d) { return d.monthName; }));
		...
		// .style("fill", "#00EEF0");
		this.bars
			.on('mousemove',  function  (s)  {         //const percent = (Math.abs(s.abs / this.total) * 100).toFixed(2) + '%';
		...
		//var sum = d3.sum(data, function(d:any) { return d.avgTime; }); 
